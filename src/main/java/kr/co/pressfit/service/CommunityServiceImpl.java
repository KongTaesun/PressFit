...
import kr.co.pressfit.dao.CommunityDAO;
...
public class CommunityServiceImpl implements CommunityService {
    ...
    public void increaseViewcnt(int idx, HttpSession session) throws Exception {
        ...
        if(current_time - update_time > 5*1000){
        	communityDAO.increaseViewcnt(idx);
    ...
    public void addAttach(String fullName) {
        communityDAO.addAttach(fullName);
    }
    // 게시글의 첨부파일 목록
    @Override
    public List<String> getAttach(int idx) {
        return communityDAO.getAttach(idx);
    }
    // 게시글의 첨부파일 목록
    @Override
    public void updateAttach(String fullName, int idx) {
        communityDAO.updateAttach(fullName,idx);
    }
    // 게시글의 첨부파일 삭제 처리
    @Override
    public void deleteFile(String fullname) {
    	communityDAO.deleteFile(fullname);
    }
    // 1. 댓글 입력
    @Override
    public void replycreate(ReplyVO vo) {
    	communityDAO.replycreate(vo);
    }
    // 1. 댓글 입력
    @Override
    public void replycreatecmt(ReplyVO vo) {
    	communityDAO.replycreatecmt(vo);
    }
    // 2. 댓글 목록
    @Override
    public List<ReplyVO> replylist(Integer boardno, int start, int end, HttpSession session) {
        List<ReplyVO> items = communityDAO.replylist(boardno, start, end);
        // 세션에서 현재 사용자 id값 저장
    ...
    public ReplyVO replydetail(Integer idx) {
        return communityDAO.replydetail(idx);
    }
    // 4. 댓글 수정
    @Override
    public void replyupdate(ReplyVO vo) {
    	communityDAO.replyupdate(vo);
    }
    // 5. 댓글 삭제
    @Override
    public void replydelete(Integer boardno) {
    	communityDAO.replydelete(boardno);
    }
    // 6. 댓글 갯수
    @Override
    public int replycount(Integer boardno) {
        return communityDAO.replycount(boardno);
    }
}